(ns shosha.operation
  "OperationActor -- one general-trading operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (ShoshaAdvisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Shosha Trading Governor (:govern) and
  ...
  One graph run = one general-trading operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A trade-order's
  ...
  `:shipment/dispatch`/`:invoice/settle` ALWAYS reach this node when
  the governor is clean -- see `shosha.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [shosha.shoshaadvisor :as shoshaadvisor]
...
(defn build
  ...
  `shosha.store/Store`).
  opts:
    :advisor      -- a `shosha.shoshaadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (shoshaadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ShoshaAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (shoshaadvisor/-advise advisor store request)]
            {:proposal p :audit [(shoshaadvisor/trace request p)]})))

      ;; Shosha Trading Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
